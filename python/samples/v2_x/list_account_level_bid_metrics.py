...
"""This example lists account-level bid metrics.

Additional RTB Troubleshooting resources that can be similarly used to retrieve
metrics for real-time bidding can be found nested under the
bidders.accounts.filterSets resource.
"""
...
def main(ad_exchange_buyer, filter_set_name, page_size):
  ...
  while more_pages:
    try:
      ...
      response = (ad_exchange_buyer.bidders().accounts().filterSets()
          .bidMetrics().list(filterSetName=filter_set_name, pageSize=page_size,
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      help=('The resource ID of the bidders.accounts resource for which the '
            ...
            '.bidMetrics/list#body.PATH_PARAMETERS.filter_set_name'))
  parser.add_argument(
  ...
  main(service, _FILTER_SET_NAME_TEMPLATE % (
      args.bidder_resource_id, args.account_resource_id,
      args.filter_set_resource_id), args.page_size)
