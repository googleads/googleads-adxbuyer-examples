...
"""Utilities used by the Authorized Buyers Ad Exchange Buyer API II Samples."""
...
MAX_PAGE_SIZE = 50
# Ad Exchange Buyer II API authorization scope.
SCOPE = 'https://www.googleapis.com/auth/adexchange.buyer'
ADEXCHANGEBUYERII_VERSIONS = ('v2beta1',)
# Set the default version of Ad Exchange Buyer II API to use.
DEFAULT_VERSION = ADEXCHANGEBUYERII_VERSIONS[-1]
...
def GetService(version=DEFAULT_VERSION):
  """Builds the adexchangebuyer2 service used for the REST API.

  Args:
    version: a str indicating the Ad Exchange Buyer II API version to be
        retrieved.
  ...
  if version in ADEXCHANGEBUYERII_VERSIONS:
    # Initialize client for Ad Exchange Buyer II API
    service = build(_API_NAME, version, credentials=credentials)
  else:
    supported_versions = ', '.join(ADEXCHANGEBUYERII_VERSIONS)
