...
#
# Lists the filter sets for a given account.
#
# Tags: Bidders.Accounts.FilterSets.list
...
def list_account_level_filter_sets(ad_exchange_buyer, owner_name, page_size)
  begin
    response = ad_exchange_buyer.list_bidder_account_filter_sets(
    ...
    unless response.filter_sets.nil?
      ...
      response.filter_sets.each do |filter_set|
        ...
        unless filter_set.creative_id.nil?
          puts "\tCreative ID: %s" % filter_set.creative_id
        end
        unless filter_set.deal_id.nil?
          puts "\tDeal ID: %s" % filter_set.deal_id
        end
      ...
      puts 'No filter sets found for account %s.' % owner_name
    end
...
if __FILE__ == $0
  ...
  owner_name = 'bidders/%s/accounts/%s' % [
      opts['bidder_resource_id'],
      opts['account_resource_id']
  ]

  list_account_level_filter_sets(service, owner_name, opts['max_page_size'])
end
