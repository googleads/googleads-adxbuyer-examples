...
#
# Lists bid metrics for a given bidder.
#
# Tags: Bidders.FilterSets.BidMetrics.list
...
def list_bidder_level_bid_metrics(
    ad_exchange_buyer, filter_set_name, page_size)
  begin
    page_token = nil

    puts 'Found the following bidder-level bid metrics with filter set %s:' %
        filter_set_name

    begin
      response = ad_exchange_buyer.list_bidder_filter_set_bid_metrics(
          filter_set_name, page_size: page_size, page_token: page_token
      )
...
if __FILE__ == $0
  ...
  filter_set_name = 'bidders/%s/filterSets/%s' %
      [opts['bidder_resource_id'], opts['filter_set_resource_id']]

  list_bidder_level_bid_metrics(
