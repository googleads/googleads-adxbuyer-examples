...
namespace Google.Apis.AdExchangeBuyer.Examples.v2_x
{
    ...
    public class CreateBidderLevelFilterSet : ExampleBase
    {
        ...
        public static void Main(string[] args)
        {
            ...
            ExampleBase example = new CreateBidderLevelFilterSet();
        ...
        {
            get { return "This code example creates a new bidder-level filter set for a given " +
                    "bidder resource ID."; }
        }
        ...
        public override void Run(BaseClientService service)
        {
            ...
            {
                Name = string.Format(
                    "{0}/filterSets/{1}", ownerName, resourceId),
            ...
            BiddersResource.FilterSetsResource.CreateRequest request =
               adXService.Bidders.FilterSets.Create(filterSet, ownerName);
            request.IsTransient = isTransient;
            FilterSet response = request.Execute();

            Console.WriteLine("Created new bidder-level filter set for owner \"{0}\":", ownerName);
