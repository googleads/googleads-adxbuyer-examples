...
/**
 * This sample illustrates how to retrieve all Account-level Filter Sets.
 */
public class GetAllAccountLevelFilterSets extends BaseSample {
  @Override
  ...
  public String getName() {
    return "Get All Account-level Filter Sets.";
  }

  @Override
  public String getDescription() {
    return "Lists Filter Sets associated with the given Account.";
  }
  ...
  public void execute(AbstractGoogleJsonClient client) throws IOException {
    ...
    String accountResourceId = getStringInput("accountResourceId",
        "Enter the Account's resource ID");
    String ownerName = String.format("bidders/%s/accounts/%s", bidderResourceId, accountResourceId);
    List<FilterSet> allFilterSets = adXClient.bidders().accounts().filterSets().list(ownerName)
    ...
    if (allFilterSets != null && allFilterSets.size() > 0) {
      ...
      System.out.printf("No Filter Sets were found associated with Account \"%s\"%n", ownerName);
    }
  }
