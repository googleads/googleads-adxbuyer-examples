...
/**
 * This sample illustrates how to retrieve all Account-level Bid Metrics.
 */
public class GetAccountLevelBidMetrics extends BaseSample {
  @Override
  ...
  public String getName() {
    return "Get Account-level Bid Metrics.";
  }

  @Override
  public String getDescription() {
    return "Lists Bid Metrics for the specified Account using the given Filter Set.";
  }
  ...
  public void execute(AbstractGoogleJsonClient client) throws IOException {
    ...
    String accountResourceId = getStringInput("accountResourceId",
        "Enter the Accounts's resource ID");
    String filterSetResourceId = getStringInput("filterSetResourceId",
        "Enter the Filter Set's resource ID");
    String filterSetName = String.format("bidders/%s/accounts/%s/filterSets/%s", bidderResourceId,
        accountResourceId, filterSetResourceId);
    ...
    do {
      ListBidMetricsResponse response = adXClient.bidders().accounts().filterSets().bidMetrics()
